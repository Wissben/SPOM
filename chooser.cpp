...
void Chooser::moyenne_Reccur(string path, double alpha,Selector* s)
{
...
void Chooser::moyenne_Arith(std::string path ,Selector*s )
{
    Mat tmp;
    Mat X;
    Mat foreGround;
    ...
    tmp = Chooser::getBackGroundRGB_8UC3(path,s);
...
void Chooser::SAP(string path, int multiple, double alpha,Selector* s)
{
    VideoCapture vc(path);
    Mat I;
    Mat Moy;
...
cv::Mat Chooser::getBackGroundRGB_8UC3(std::string path,Selector* s)
{
    VideoCapture vc(path);
    Mat X;
    Mat M;
